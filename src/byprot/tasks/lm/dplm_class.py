...
class ConditionalDPLMTrainingTask(TaskLitModule):
    ...
    @torch.no_grad()
    def inject_noise(
        self, tokens, noise=None, sel_mask=None, mask_by_unk=False
    ):
        ...
        def _random_mask(target_tokens):
            target_masks = target_tokens.ne(padding_idx)
    ...
    def step(self, batch):
        ...
        TODO update doc comment to new batch format

        coord conditioning used:
        ...
        class conditioning will need:
        - class_ids: FloatTensor [bsz, len, n_atoms, 3], coordinates of proteins
        - lengths: int [bsz, len], protein sequence lengths
        - tokens: LongTensor [bsz, len], sequence of amino acids
        """
