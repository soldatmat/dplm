...
from byprot.datamodules.dataset.martsdb_class import (
    MartsDBClassDataset,
    setup_dataloader,
    Subset,
)
...
@register_datamodule("martsdb_class")
class MartsDBClassDataModule(LightningDataModule):
    def __init__(
        self,
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage == "fit":
            self.train_dataset = MartsDBClassDataset(
            ...
            self.test_dataset = MartsDBClassDataset(
                csv_file=self.hparams.data_path,
                split=None,
                max_len=self.hparams.max_len,
                class_column=self.class_column,
