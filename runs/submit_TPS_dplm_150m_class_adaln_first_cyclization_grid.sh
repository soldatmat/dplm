...
#
# Syntax:
#   [VAR=value ...] bash runs/submit_TPS_dplm_150m_class_adaln_first_cyclization_grid.sh
#
# Examples:
#   DRY_RUN=1 bash runs/submit_TPS_dplm_150m_class_adaln_first_cyclization_grid.sh
#   CLUSTER=karolina DRY_RUN=1 bash runs/submit_TPS_dplm_150m_class_adaln_first_cyclization_grid.sh
#   CLUSTER=karolina SCHEDULER_TYPE=slurm bash runs/submit_TPS_dplm_150m_class_adaln_first_cyclization_grid.sh
#
...
EXP="${EXP:-tps/TPS_dplm_150m_class_adaln_first_cyclization}"
RUN_PREFIX="${RUN_PREFIX:-TPS_dplm_150m_class_adaln_first_cyclization_grid}"
...
# adaLN trains only the adaLN-single module + class encoder on a FROZEN backbone:
# no LoRA. (LoRA can still be re-enabled via LORA_ENABLE_LIST=true if desired.)
...
submit_job() {
    ...
    job_name="cpl${lr_code}w${warmup_code}t${total_code}s${save_code}v${val_code}e${lr_end_code}i${warmup_init_code}${lora_code}m${ltm_idx}"
